...
class Settings(BaseSettings):
    ...
    # ==================== DeepSeek LLM (默认主 provider) ====================
    deepseek_api_key: str = Field(
        default="",
        description="DeepSeek API Key (platform.deepseek.com). 作为默认 LLM provider.",
    )
    ...
    deepseek_chat_model: str = Field(
        default="deepseek-v4-pro", description="默认 Chat 模型 (高质量)"
    )
    deepseek_router_model: str = Field(
        default="deepseek-v4-flash", description="Router / Planner 快模型"
    )

    # ==================== DashScope LLM (备选) ====================
    # 当模型名以 "qwen" 开头时, get_chat_llm 会自动切到 DashScope 的 base_url + api_key.
    dashscope_api_key: str = Field(default="", description="DashScope API Key (备选 provider)")
    dashscope_base_url: str = Field(
        default="https://dashscope.aliyuncs.com/compatible-mode/v1",
        description="DashScope OpenAI 兼容模式 URL",
    )
    dashscope_chat_model: str = Field(default="qwen-max", description="DashScope Chat 模型")
    dashscope_router_model: str = Field(default="qwen-turbo", description="DashScope Router 模型")

    # ==================== 本地 Embedding (BGE) ====================
    local_embedding_model: str = Field(
        default="BAAI/bge-small-zh",
        description="sentence-transformers 模型名 (本地 Embedding)",
    )
    local_embedding_dim: int = Field(
        default=512, description="Embedding 向量维度 (BGE-small-zh = 512)"
    )
    local_embedding_device: str = Field(
        default="cpu", description="Embedding 推理设备 (cpu / cuda:0)"
    )
    ...
    rag_rerank_enabled: bool = Field(
        default=True,
        description=(
            "是否启用 Reranker (本地 BGE CrossEncoder). "
            "为什么: 向量相似度 ≠ 问答相关性, reranker 是 cross-encoder 能精细打分; "
            "局限: 首次加载需下载模型, CPU 推理有一定延迟, 异常时会自动降级."
        ),
    )
    rag_rerank_model: str = Field(
        default="BAAI/bge-reranker-base",
        description=(
            "本地 Reranker 模型名 (sentence-transformers CrossEncoder). "
            "为什么默认 BAAI/bge-reranker-base: 与 BGE embedding 同家族, 本地推理零成本; "
            "局限: 需下载模型 (~1GB), CPU 推理约 50-200ms/对."
        ),
    )
    ...
    agent_executor_model: str = Field(
        default="",
        description=(
            "Executor 使用的模型. 留空则走 deepseek_chat_model. "
            "建议配 deepseek-v4-flash: Executor 每步都要 LLM, 跑得最频繁, "
            "用 pro 会偏慢. 报告综合那一步用 pro 最划算."
        ),
    )
    agent_report_model: str = Field(
        default="",
        description=(
            "最终报告合成使用的模型. 留空则走 deepseek_chat_model (推荐 pro). "
            "Replanner 用 flash 做快速决策, 决策 is_finished=true 后再用 report_model "
            "单独写一份高质量报告, 质量 / 速度两头兼顾."
        ),
    )
    agent_planner_model: str = Field(
        default="",
        description=(
            "Planner / Replanner 使用的模型. 留空则走 deepseek_router_model (flash). "
    ...
    local_llm_enabled: bool = Field(
        default=False,
        description="是否启用本地 LLM 兜底 (主 LLM 不可达时自动切换)",
    )
    local_llm_force: bool = Field(
        default=False,
        description="强制使用本地 LLM (跳过云端), 适合无 API key 或纯离线开发",
    )
    ...
    local_llm_probe_host: str = Field(
        ...
        description="探测主 LLM 是否可达的目标域名 (TCP 443)",
    )
    ...
    def validate_runtime(self) -> None:
        ...
        """
        if not self.deepseek_api_key or self.deepseek_api_key.startswith("your-"):
            raise RuntimeError(
                "DEEPSEEK_API_KEY 未配置. 请编辑 .env 文件填入真实 API key. "
