...
设计要点:
  - 默认使用 DeepSeek (OpenAI 兼容模式), 直接复用 langchain_openai.ChatOpenAI
  - 模型名以 "qwen" 开头时自动切到 DashScope
...
def get_chat_llm(
    ...
    **kwargs: Any,
    ...
    Args:
        model: 模型名 (None 则用 settings.deepseek_chat_model)
    ...
    if _should_use_local_llm():
        ...
        # 本地模型不需要 enable_thinking/thinking 等云端专属参数
    ...
    selected_model = model or settings.deepseek_chat_model

    # ===== 模型名以 qwen 开头 → 路由到 DashScope =====
    ...
    if not api_key:
        logger.warning(
            "[LLM] DeepSeek API key 未配置, 仍尝试调用 (会 401)"
        )
    return ChatOpenAI(
        model=selected_model,
