...
async def _synthesize_final_report(
    ...
    """
    report_model = settings.agent_report_model or settings.deepseek_chat_model
    decide_model = settings.agent_planner_model or settings.deepseek_router_model
...
async def replan_node(state: PlanExecuteState) -> PlanExecuteState:
    ...
    # ===== 调用 LLM 决策 =====
    # Replanner 用快模型: 决策本身只需要看进度+输出结构化 Plan/Response.
    replanner_model = settings.agent_planner_model or settings.deepseek_router_model
