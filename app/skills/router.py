...
职责:
  - 输入: state.input (用户告警/问题)
  - 输出: state.selected_skill (skill name) + state.skill_reason (一句话理由)
  - 写入位置: LangGraph state, 由 planner 节点消费

策略 (第一版):
  - 纯 LLM + structured output (with_structured_output)
  - LLM 在 Registry 现有 Skill 列表中选一个 name
  - LLM 返回菜单外的 name 时, 回退到 generic_oncall
  - LLM 调用失败时, 也回退到 generic_oncall

兜底原则: 任何异常都不能阻塞流程, 永远有一个可用的 Skill.

依赖说明:
  - Prompt 用本包的 app.skills.prompts (不引用 app.agents.*),
    保证依赖单向 (agents -> skills), 不会循环.
  - PlanExecuteState 仅作类型注解, 用 TYPE_CHECKING 避免运行时触发 app.agents.__init__.
"""
...
from app.skills.prompts import SKILL_ROUTER_SYSTEM_PROMPT, SKILL_ROUTER_USER_TEMPLATE
...
class SkillChoice(BaseModel):
    ...
    is_oncall: bool = Field(
        default=True,
        description="用户输入是否属于 OnCall/运维诊断范围",
    )
    skill_name: str = Field(
        ...,
        description="选中的 Skill name (snake_case), 必须是给定菜单中已存在的项",
    )
    confidence: float = Field(
        default=0.0,
        ge=0.0,
        le=1.0,
        description="路由置信度, 0 到 1",
    )
    reason: str = Field(
        default="",
        description="一句话说明为什么选这个 Skill, 用于可观测",
    )
...
async def skill_router_node(state: PlanExecuteState) -> PlanExecuteState:
    """选 Skill 节点: 输入 state.input, 输出 state.selected_skill."""
    ...
    messages = [
        ...
        {
            "role": "user",
            "content": SKILL_ROUTER_USER_TEMPLATE.format(
                menu=menu,
                input=user_input,
    ...
    if chosen not in available:
        logger.warning(
            f"[Router] LLM 返回不存在的 skill {chosen!r}, 回退到 {GENERIC_SKILL_NAME}"
        )
